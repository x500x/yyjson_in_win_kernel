...
fopen(const char *path, const char *mode)
{
    ...
    if (!NT_SUCCESS(status)) {
        yyjsonk_logf(DPFLTR_INFO_LEVEL,
                     "[example][file] path convert failed path=%s status=0x%08X\n",
                     path,
                     status);
        free(file);
        return NULL;
    ...
    if (!NT_SUCCESS(status)) {
        yyjsonk_logf(DPFLTR_INFO_LEVEL,
                     "[example][file] open failed path=%s status=0x%08X\n",
                     path,
                     status);
        free(file);
        return NULL;
    ...
    while (total < requested) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            yyjsonk_logf(DPFLTR_ERROR_LEVEL,
                         "[example][file] read failed status=0x%08X offset=%llu size=%lu\n",
                         status,
